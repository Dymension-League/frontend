...
import tokenData
  from "../../artifacts/proofs/proofs_0xcba72fb67462937b6fa3a41e7bbad36cf169815ea7fe65f8a4b85fd8f5facb28.json";
...
const useMintService = () => {
  ...
  const getTokenMetadata = async (tokenId: number): Promise<any> => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch metadata from URI: ${tokenURI}. Status: ${response.status}`);
      }
      ...
      console.error(`Error fetching token metadata for token ID ${tokenId}:`, error);
      throw error;
    }
  };
  ...
  const getIPFSTokenMetadata = async (tokenId: number): Promise<any> => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch metadata for token ID ${tokenId}. Status: ${response.status}`);
      }
      ...
      const tokenMetadata = {
        ...
        img: imgUrl,
        type: metadata.attributes.find((attr: any) => attr.trait_type === "Type")?.value || "Unknown",
        model: metadata.attributes.find((attr: any) => attr.trait_type === "Model")?.value || "Unknown",
        color: metadata.attributes.find((attr: any) => attr.trait_type === "Color")?.value || "Unknown",
        tool: metadata.attributes.find((attr: any) => attr.trait_type === "Tool")?.value || "Unknown",
        capacity: Number(metadata.attributes.find((attr: any) => attr.trait_type === "Capacity")?.value) || 0,
        attack: Number(metadata.attributes.find((attr: any) => attr.trait_type === "Attack")?.value) || 0,
  ...
  const getIPFSTokenMetadataBatch = async (tokenIds: number[]): Promise<any[]> => {
    const metadataPromises = tokenIds.map(tokenId => getIPFSTokenMetadata(tokenId));
